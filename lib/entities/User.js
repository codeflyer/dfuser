...
var ExpireToken = require('./values/ExpireToken');
var Q = require('q');
...
User.prototype.resetRoles = function(acl) {
  ...
  if (!this.isLoad()) {
    deferred.reject('User not initialized');
    ...
    acl.removeUserRoles(that.getId(), roles, function(err) {
      if (err) {
        ...
        acl.addUserRoles(that.getId(), that.getRoles(), function(err) {
          ...
          deferred.resolve(that.getRoles());
        });
      }
    });
  });
 ...
 */
User.prototype.permanentDelete = function() {
  var deferred = Q.defer();
  var that = this;
  PubSub.publish('user:beforeUserRemove', that, true).then(
      function() {
